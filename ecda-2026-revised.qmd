...
- A request for policy information and a request for applicant ranking may reach the same approved model endpoint.
- The issue does not require malicious intent. An employee may simply be unaware that a request falls under a regulated use case.
- Policies, training, and access controls remain necessary, but they do not govern every request entered into an open text field.
- This creates the need for a request-level signal before the model produces an answer.
:::

### Risk categories in the EU AI Act {#ai-act-risk-categories}

![Authors’ illustration based on the legal sources cited below.](img/fig_ai_act_tiers.svg){width=94% fig-align="center" fig-alt="EU AI Act risk pyramid with four categories, legal basis, main obligations, dates of application, and possible routing actions."}

::: {.notes}
**Say**

- The figure summarises the four categories. I will only use it to establish the regulatory vocabulary for the rest of the talk.
- Article 5 covers prohibited practices. Annex III lists high-risk areas of use. Article 50 contains transparency duties.
- The category follows from the intended purpose and use of an AI system. The next slide explains why a prompt cannot establish that category on its own.
- The figure also shows possible organisational responses. The AI Act itself does not prescribe prompt-level routing.
:::
...
The category follows from the AI system’s **intended purpose and use** [@aiact].
...
- The AI Act distinguishes four regulatory categories. These categories depend on the intended purpose and actual use of an AI system.
- The emotion-recognition example shows why prompt classification cannot reproduce a legal assessment. The same request can describe a harmless demonstration or a prohibited use at work or in education.
- We observe the request text, but usually not the deployment setting or affected people.
- The model therefore provides a screening signal. It can trigger a request for more context or a more constrained response policy.
- The detailed legal overview and the full context diagram are available in the backup.
:::
...
- show that embeddings with small classifiers can support fast screening [@embinject; @bertguard; @reflexguard]
...
- Existing prompt guardrails address related but different classification problems.
- Published studies focus on attacks or unsafe content, and their positive classes are much more common than the severe cases in our corpus.
- They support the use of dense embeddings and compact classifiers for fast screening.
- Their performance figures do not answer our research question because the label meaning, prevalence, data, and evaluation conditions differ.
- Our claim remains narrow: we evaluate a prompt-level screening signal, not a complete legal compliance process.
- The detailed comparison of the three studies is available in the backup.
:::
...
- **3** public conversation datasets [@lmsys; @isotonic; @ultrachat]
...
- Our first idea was to generate the complete classifier corpus with an LLM.
- The approach gave us targeted examples with apparent labels, but the examples often repeated similar language and scenarios. The labels still needed verification.
- We therefore use observed prompts from three public conversation datasets for classifier training and evaluation.
- Filtering and deduplication leave 6.72 million prompts. Experts labelled 826 prompts in the first round, including only one Unacceptable case.
- Synthetic variants have one narrow role: they help the embedding retriever find observed prompts near reviewed rare cases. They never enter classifier training or evaluation.
- The detailed comparison and source-pool description are available in the backup.
:::
...
![Authors’ illustration of the study workflow.](img/fig_labelling_loop.svg){width=96% fig-align="center" fig-alt="An iterative labelling process combines seed labels, embedding-based retrieval, two language-model labels, human review, and retriever fine-tuning over eleven rounds."}
...
- Disagreement cases provide the strongest learning signal. AutoResearch used them to refine the labelling prompt automatically.
- The retriever is fine-tuned only for finding the next candidates. It is not the final classifier.
- In round 11, 89 synthetic prompts derived from the AI Act were additional retrieval anchors. They are not observed corpus prompts and never enter the classifier data.
...
- The largest-probability rule fails because the Minimal category dominates. We therefore select separate thresholds for the rare categories.
...
- The screening layer runs before model inference. If several thresholds are exceeded, the most severe category determines the response.
- Rule checks are conservative: they may raise the response level, but cannot lower it.
...
slide_table(
  data.frame(
    Encoder = c("Base ModernBERT", "fine-tuned encoder, Run 5", "fine-tuned encoder, Run 11"),
...
- Run 5 and Run 11 refer to checkpoints from the fifth and eleventh rounds of the retrieval loop.
...
- Severe AP evaluates the ranking before threshold selection. Macro-F1 uses the largest-probability class as a secondary metric.
- The separate class thresholds are selected later for the operating point, as described on the Evaluation protocol slide.
...
slide_table(
  data.frame(
    Target = c("four categories", "severe versus other"),
...
slide_table(
  data.frame(
    ...
    `Difference from RBF SVM` = c("reference", "−0.037", "−0.095", "−0.333", "−0.342"),
    check.names = FALSE
  ),
  align = c("l", "r", "r"),
  bold_rows = 1
...
- The 95% bootstrap interval for this improvement is from 0.089 to 0.155. The binary model performs better in all 2,000 paired resamples.
- Every classifier in the right-hand table uses this binary target, the same embeddings, and the same cross-validation folds.
- The RBF SVM performs best. Its lead over tuned LightGBM is 0.037, while both linear models reach only about 0.30 average precision.
...
![Source: authors’ analysis.](img/fig_learning_curve.svg){width=88% fig-align="center" fig-alt="Learning curve for severe prompt classification. Four observed training sizes show increasing average precision; the dashed continuation beyond 400 examples is an extrapolation."}
...
- The learning curve estimates how much additional severe labels may help.
- Severe examples are subsampled only in the training sets. Every test set retains all positive cases. Each point averages five random subsamples, and the bars show one standard deviation.
- Average precision continues to increase over the observed range. The fitted relationship implies about 0.09 additional AP when the training set doubles.
- The estimate of about 1,160 severe examples for AP 0.75 extrapolates beyond the largest observed training set of 400. It supports planning but does not guarantee performance.
- We already searched 6.7 million prompts through eleven targeted rounds, but the consensus corpus contains only 593 severe examples.
- The 2,316 earlier disagreement cases offer a more informative next sample than additional easy examples. Half received at least one severe vote, and an initial human sample contained 26 percent severe cases.
- The next study should use double annotation and explicit adjudication. Reviewers should first agree on the appropriate policy action, then on the exact legal category.
- The detailed learning-curve explanation and the full monitoring loop are available in the backup.
:::
...
- Experts labelled an initial seed of 826 observed prompts. Only one was Unacceptable, which shows why random sampling alone is not viable.
- The retrieval pool grew after every round. By round 11, 9,127 labelled prompts were available as search seeds.
:::
...
- We derived 663 variants from 15 reviewed Unacceptable corpus examples for retrieval training. This helped retrieve 52 observed Unacceptable prompts in total.
- Both these variants and the 89 synthetic AI Act anchors are retrieval aids only. They do not enter classifier training or evaluation.
:::
